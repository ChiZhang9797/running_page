...
fractions = {  # name:   style_attrs, frac_attrs
    "frac": ({}, {}),
    "cfrac": (
        {"displaystyle": True, "scriptlevel": 0, "CLASS": "cfrac"},
        {},
    ),  # in LaTeX with padding
...
class math:
    ...
    def __repr__(self):
        ...
        content += [
            "%s=%r" % (k, v) for k, v in self.attributes.items() if v is not None
        ]
...
def handle_cmd(name, node, string):  # noqa: C901 TODO make this less complex
    ...
    if name in bigdelimiters:
        ...
        if delimiter not in "()[]/|.":
            try:
                ...
                raise SyntaxError(
                    'Unsupported "\\%s" delimiter "%s"!' % (name, delimiter)
                )
        if size:
    ...
    if name in layout_styles:  # 'displaystyle', 'textstyle', ...
        ...
        if isinstance(node, mrow) and len(node) == 0:
            ...
            raise SyntaxError(
                'Declaration "\\%s" must be first command ' "in a group." % name
            )
        return new_node, string
...
def handle_script_or_limit(node, c, limits=""):
    ...
    if c == "_":
        if isinstance(child, mover):
            ...
            limits in ("limits", "movablelimits")
            or limits == ""
            and child.get("movablelimits", None)
        ):
