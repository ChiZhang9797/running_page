...
class Writer(writers.Writer):
    ...
    def apply_template(self):
        with open(
            self.document.settings.template, "r", encoding="utf-8"
        ) as template_file:
...
class HTMLTranslator(nodes.NodeVisitor):
    ...
    def visit_footnote(self, node):
        ...
        self.body.append(
            self.starttag(
                node, "aside", classes=[node.tagname, label_style], role="note"
            )
        )
        ...
        if self.image_loading == "embed":
            try:
                ...
                self.document.reporter.error(
                    "Cannot embed image %r: %s" % (uri, err.strerror)
                )
            else:
        ...
        if self.math_output == "mathjax" and not self.math_header:
            ...
            try:
                if converter == "latexml":
                    ...
                    self.document.reporter.error(
                        'option "%s" not supported ' 'with math-output "MathML"'
                    )
            except OSError:
