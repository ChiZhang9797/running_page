...
def haversine_distance(
    ...
    a = mod_math.pow(mod_math.sin(d_lat / 2), 2) + mod_math.pow(
        mod_math.sin(d_lon / 2), 2
...
def get_course(
    ...
    if not loxodromic:
        ...
        x = mod_math.cos(lat1) * mod_math.sin(lat2) - mod_math.sin(lat1) * mod_math.cos(
            lat2
        ) * mod_math.cos(d_lon)
    else:
        ...
        delta = mod_math.pi / 4
        x = mod_math.log(
            mod_math.tan(delta + 0.5 * lat2) / mod_math.tan(delta + 0.5 * lat1)
        )

    course = mod_math.degrees(mod_math.atan2(y, x))
    return (course + 360.0) % 360.0
...
def calculate_max_speed(
    ...
    standard_distance_deviation = mod_math.sqrt(
        sum(map(lambda distance: (distance - average_distance) ** 2, distances))
        / float(size)
    )

    # Ignore items where the distance is too big:
    filtered_speeds_and_distances = filter(
        lambda speed_and_distance: abs(speed_and_distance[1] - average_distance)
        <= standard_distance_deviation * 1.5,
    ...
    speeds = list(
        map(
            lambda speed_and_distance: speed_and_distance[0],
...
def distance(
    latitude_1: float,
    longitude_1: float,
    elevation_1: Optional[float],
    latitude_2: float,
    longitude_2: float,
    elevation_2: Optional[float],
    ...
    if haversine or (
        abs(latitude_1 - latitude_2) > 0.2 or abs(longitude_1 - longitude_2) > 0.2
    ):
...
def elevation_angle(
    location1: "Location", location2: "Location", radians: float = False
) -> Optional[float]:
...
def distance_from_line(
    point: "Location", line_point_1: "Location", line_point_2: "Location"
) -> Optional[float]:
    ...
    if a is not None and b is not None and c is not None:
        s = (a + b + c) / 2.0
        return 2.0 * mod_math.sqrt(abs(s * (s - a) * (s - b) * (s - c))) / a
    return None
...
def get_line_equation_coefficients(
    ...
    if location1.longitude == location2.longitude:
        ...
        a = float(location1.latitude - location2.latitude) / (
...
def simplify_polyline(
    points: List["Location"], max_distance: Optional[float]
) -> List["Location"]:
...
class Location:
    ...
    def elevation_angle(
        self, location: "Location", radians: bool = False
    ) -> Optional[float]:
...
class LocationDelta:
    ...
    def __init__(
        ...
        longitude_diff: Optional[float] = None,
    ) -> None:
        """
        Version 1:
            Distance (in meters).
    ...
    def move_by_angle_and_distance(self, location: Location) -> Tuple[float, float]:
        ...
        vertical_distance_diff = (
            mod_math.sin(mod_math.radians(90 - self.angle_from_north)) / ONE_DEGREE
        )
        horizontal_distance_diff = (
            mod_math.cos(mod_math.radians(90 - self.angle_from_north)) / ONE_DEGREE
        )
    ...
    def move_by_lat_lon_diff(self, location: "Location") -> Tuple[float, float]:
        return (
            location.latitude + self.latitude_diff,
