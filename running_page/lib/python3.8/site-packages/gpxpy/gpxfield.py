...
class FloatConverter:
    def __init__(self) -> None:
        self.from_string = (
            lambda string: None if string is None else float(string.strip())
        )
...
class IntConverter:
    def __init__(self) -> None:
        self.from_string = (
            lambda string: None if string is None else int(string.strip())
        )
...
class AbstractGPXField:
    def __init__(
        self, attribute_field: Optional[str] = None, is_list: Optional[bool] = None
    ) -> None:
...
class GPXField(AbstractGPXField):
    ...
    def __init__(
        ...
        type: Any = None,
        possible: Optional[Iterable[str]] = None,
    ...
    def from_xml(self, node: Any, version: str) -> Any:
        ...
        if result is None:
            if self.mandatory:
                ...
                raise mod_gpx.GPXException(
                    f"{self.name} is mandatory in {self.tag} (got {result})"
                )
            return None
        ...
        if self.type_converter:
            try:
                ...
                raise mod_gpx.GPXException(
                    f"Invalid value for <{self.tag}>... {result} ({e})"
                )
        ...
        if self.possible:
            if not (result in self.possible):
                ...
                raise mod_gpx.GPXException(
                    f'Invalid value "{result}", possible: {self.possible}'
                )
...
class GPXComplexField(AbstractGPXField):
    def __init__(
        self, name: str, classs: Any, tag: Optional[str] = None, is_list: bool = False
    ) -> None:
    ...
    def to_xml(
        ...
        if self.is_list:
            ...
            return gpx_fields_to_xml(
                value, self.tag, version, prettyprint=prettyprint, indent=indent
            )
...
class GPXExtensionsField(AbstractGPXField):
    ...
    def __init__(
        self, name: str, tag: Optional[str] = None, is_list: bool = True
    ) -> None:
        AbstractGPXField.__init__(self, is_list=is_list)
        self.name = name
    ...
    def _ETree_to_xml(
        ...
        for child in node:
            result.append(
                self._ETree_to_xml(
                    child, nsmap, prettyprint=prettyprint, indent=indent + "  "
                )
            )
    ...
    def to_xml(
        ...
        for extension in value:
            result.append(
                self._ETree_to_xml(
                    extension, nsmap, prettyprint=prettyprint, indent=indent + "  "
                )
            )
        result.append("\n" + indent + "</" + self.tag + ">")
        return "".join(result)
    ...
    for gpx_field in fields:
        ...
        if isinstance(gpx_field, str):
            ...
            if suppressuntil:
                ...
                suppressuntil, gpx_field = _check_dependents(instance, gpx_field)
            ...
            if gpx_field.attribute:
                ...
                if tag_open:
                    body.append(">")
                    tag_open = False
                xml_value = gpx_field.to_xml(
                    value, version, nsmap, prettyprint=prettyprint, indent=indent + "  "
                )
                if xml_value:
...
def gpx_check_slots_and_default_values(classs: Callable[[], Any]) -> None:
    ...
    try:
        ...
        attributes = list(
            filter(lambda x: not callable(getattr(instance, x)), attributes)
        )
    ...
    if attributes != slots:
        raise Exception(
            f"Attributes for {classs.__name__} is\n{attributes} but should be\n{slots}"
        )
    ...
    for field in fields:
        if not isinstance(field, str):
            ...
            if (
                ...
                raise Exception(
                    "Invalid default value %s.%s is %s but should be %s"
