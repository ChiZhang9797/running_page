...
class ArcGIS(Geocoder):
    ...
    def geocode(
        self, query, *, exactly_one=True, timeout=DEFAULT_SENTINEL, out_fields=None
    ):
        """
    ...
    def _parse_geocode(self, response, exactly_one):
        ...
        for resource in response["candidates"]:
            ...
            geocoded.append(
                Location(resource["address"], (geometry["y"], geometry["x"]), resource)
            )
        if exactly_one:
            return geocoded[0]
        return geocoded
    ...
    def reverse(
        self, query, *, exactly_one=True, timeout=DEFAULT_SENTINEL, distance=None
    ):
        """
