...
class IGNFrance(Geocoder):
        ...
        *,
        ...
        if not ((api_key and username and password) or (api_key and referer)):
            raise ConfigurationError(
                "You should provide an api key and a "
        ...
        if (username and password) and referer:
            raise ConfigurationError(
                "You can't set username/password and "
        ...
        *,
        ...
        if query_type not in ["PositionOfInterest", "StreetAddress", "CadastralParcel"]:
            raise GeocoderQueryError(
                """You did not provided a query_type the
        ...
        if query_type == "CadastralParcel" and len(query.strip()) != 14:
            raise GeocoderQueryError(
                """You must send a string of fourteen
    ...
    def _parse_xml(self, page, is_reverse=False, is_freeform=False, exactly_one=True):
        ...
        if exactly_one:
            ...
            return [
                self._parse_place(place, is_freeform=is_freeform) for place in places
            ]
    ...
    def _xml_to_json_places(self, tree, is_reverse=False):
        ...
        select_multi = (
            "GeocodedAddress" if not is_reverse else "ReverseGeocodedLocation"
        )
    ...
    def _request_raw_content(self, url, callback, *, timeout):
        ...
        return self._call_geocoder(
            url, callback, headers=headers, timeout=timeout, is_json=False
        )
