...
from geopy.geocoders.base import Geocoder, DEFAULT_FORMAT_STRING, DEFAULT_TIMEOUT
...
class GeocoderDotUS(Geocoder):  # pylint: disable=W0223
    ...
    def geocode(self, query, exactly_one=True, timeout=None):
        ...
        content = (
            page.read().decode("utf-8") if py3k else page.read()
        )  # pylint: disable=E1101,E1103
        places = [
            r
            for r in csv.reader([content] if not isinstance(content, list) else content)
        ]
        if not len(places):
            return None
        if exactly_one:
    ...
    def _parse_result(result):
        ...
        name = join_filter(
            ", ", [join_filter(" ", address), city, join_filter(" ", [state, zip_code])]
        )
    ...
    def _get_headers(self):
        ...
        if self.authenticated:
            ...
            auth = " ".join(
                ("Basic", b64encode(username_password.encode("utf-8")).decode("utf-8"))
            )
