...
class TomTom(Geocoder):
    ...
    def reverse(
        ...
        url = "?".join(
            (self.api_reverse % dict(position=quoted_position), urlencode(params))
        )
    ...
    def _parse_json(self, resources, exactly_one):
        ...
        if exactly_one:
            ...
            return [
                self._parse_search_result(result) for result in resources["results"]
            ]
    ...
    def _parse_search_result(self, result):
        ...
        return Location(
            result["address"]["freeformAddress"], (latitude, longitude), result
        )
    ...
    def _parse_reverse_json(self, resources, exactly_one):
        ...
        if exactly_one:
            ...
            return [
                self._parse_reverse_result(result) for result in resources["addresses"]
            ]
