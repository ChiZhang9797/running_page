...
class Photon(Geocoder):
        ...
        *,
        ...
        if location_bias:
            try:
                ...
                raise ValueError(
                    ("Location bias must be a" " coordinate pair or Point")
                )
        ...
        if osm_tag:
            if isinstance(osm_tag, str):
                ...
                if not isinstance(osm_tag, collections.abc.Iterable):
                    raise ValueError(
                        "osm_tag must be a string or " "an iterable of strings"
                    )
    ...
    def _parse_json(self, resources, exactly_one=True):
        ...
        if exactly_one:
            ...
            return [
                self._parse_resource(resource) for resource in resources["features"]
            ]
    ...
    def _parse_resource(self, resource):
        ...
        name = [
            resource["properties"].get(k)
            for k in name_elements
            if resource["properties"].get(k)
        ]
        location = ", ".join(name)
