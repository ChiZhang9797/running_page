...
class Geocoder:
    ...
    def __init__(
        ...
        *,
        ...
        self.timeout = (
            timeout if timeout is not DEFAULT_SENTINEL else options.default_timeout
        )
        self.proxies = (
            proxies if proxies is not DEFAULT_SENTINEL else options.default_proxies
        )
        self.headers = {"User-Agent": user_agent or options.default_user_agent}
        self.ssl_context = (
            ssl_context
            if ssl_context is not DEFAULT_SENTINEL
        ...
        self.adapter = adapter_factory(
            proxies=self.proxies, ssl_context=self.ssl_context
        )
    ...
    def _format_bounding_box(
        ...
        return output_format % dict(
            lat1=min(p1.latitude, p2.latitude),
    ...
    def _call_geocoder(
        ...
        timeout = timeout if timeout is not DEFAULT_SENTINEL else self.timeout
