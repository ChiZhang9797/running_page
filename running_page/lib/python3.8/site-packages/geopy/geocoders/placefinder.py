...
class YahooPlaceFinder(Geocoder):  # pylint: disable=W0223
    ...
    def _filtered_results(results, min_quality, valid_country_codes):
        ...
        if valid_country_codes:
            results = [
                loc for loc in results if loc.raw["countrycode"] in valid_country_codes
            ]
    ...
    def geocode(
        ...
        """
        params = {"location": query, "flags": "J"}  # JSON
        ...
        response = self._call_geocoder(
            self.api, timeout=timeout, requester=get, params=params, auth=self.auth
        )
    ...
    def reverse(self, query, exactly_one=True, timeout=None):
        ...
        return self.geocode(
            query, exactly_one=exactly_one, timeout=timeout, reverse=True
        )
