...
class URLLibAdapter(BaseSyncAdapter):
    ...
    def __init__(self, *, proxies, ssl_context):
        ...
        opener = build_opener(HTTPSHandler(context=ssl_context), ProxyHandler(proxies))
        ...
        try:
            ...
            if isinstance(error, HTTPError):
                ...
                raise AdapterHTTPError(
                    message, status_code=code, headers=response_headers, text=body
                )
            ...
            if status_code >= 400:
                response_headers = {
                    name.lower(): value for name, value in page.headers.items()
                }
                raise AdapterHTTPError(
    ...
    def session(self):
        ...
        if session is None:
            session = aiohttp.ClientSession(
                trust_env=False, raise_for_status=False  # don't use system proxies
            )
